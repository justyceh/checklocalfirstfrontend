    ...
    return (
      <div className="min-h-screen bg-surface pt-20 sm:pt-24 pb-16 px-5">
        <div className="mx-auto max-w-lg text-center pt-8">
          ...
          <Link href="/dashboard" className="mt-4 inline-block text-sm font-medium text-primary hover:underline">Go to Dashboard →</Link>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-surface pt-20 sm:pt-24 pb-16 px-5">
      <div className="mx-auto max-w-2xl">
        ...
        {/* Welcome banner */}
        <div className="mb-8 rounded-2xl bg-primary px-6 py-6 text-white">
        ...
        <section className="rounded-2xl border border-black/10 bg-white p-6 shadow-sm mb-4">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              <h2 className="font-semibold text-dark">Business Details</h2>
              <p className="text-xs text-muted">Help customers know who you are and how to reach you.</p>
            </div>
          </div>

          {saveSuccess && (
            <p className="mb-4 rounded-lg bg-primary/10 px-4 py-2.5 text-sm text-primary-dark">{saveSuccess}</p>
          )}
          ...
          <form onSubmit={handleSave} className="flex flex-col gap-4">
            ...
            <div>
              <label className={LABEL}>Address <span className="text-faint font-normal">(optional)</span></label>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                <label className={LABEL}>Phone <span className="text-faint font-normal">(optional)</span></label>
                <input className={INPUT} type="tel" value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} />
              </div>
              <div>
                <label className={LABEL}>Email <span className="text-faint font-normal">(optional)</span></label>
            ...
            <button
              type="submit"
              disabled={saving}
              className="cursor-pointer rounded-lg bg-primary py-2.5 text-sm font-semibold text-white hover:bg-primary-dark disabled:opacity-60 transition-colors"
            >
        ...
        <section className="rounded-2xl border border-black/10 bg-white p-6 shadow-sm mb-4">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              <h2 className="font-semibold text-dark">Your Services</h2>
              <p className="text-xs text-muted">Add at least one service so customers know what you offer.</p>
            </div>
          </div>
          <ServiceManager
            slug={business.slug}
        ...
        <div className="rounded-2xl border border-black/10 bg-white p-6 shadow-sm flex flex-col items-center text-center gap-3">
          <p className="text-sm text-subtle">All done? Head to your dashboard to manage your full listing.</p>
          <Link
            href="/dashboard"
            className="inline-flex items-center gap-1.5 rounded-lg bg-dark px-6 py-3 text-sm font-semibold text-white hover:bg-black transition-colors"
          >
            Go to Dashboard →
          </Link>
        </div>
